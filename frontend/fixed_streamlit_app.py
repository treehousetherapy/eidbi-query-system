...
# Add a checkbox for using the simplified direct query (no vector DB)
use_simple_query = st.checkbox("Use simple query (faster, but doesn't use document context)", value=False, 
    help="This bypasses the vector database search and just uses the general knowledge of the AI model.")
...
if st.button("Ask", type="primary"):
    if not query_text:
        ...
        with st.spinner("Finding relevant information and generating answer..." if not use_simple_query else "Generating answer..."):
            try:
                # Choose the correct endpoint based on checkbox
                if use_simple_query:
                    endpoint = f"{BACKEND_URL}/simple-answer"
                    # FIXED LINE: properly format the payload with query_text as key
                    payload = {"query_text": query_text}
                    st.info("Using simplified query without document retrieval.")
                else:
